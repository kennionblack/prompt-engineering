...
def get_talk_urls(conference_url, year, month, session):
    ...
    for link in soup.select(
        'div.talk-list a[href*="/study/general-conference/"], article a[href*="/study/general-conference/"]'
    ):
        href = link.get("href")
        ...
        try:
            ...
            if not (
                talk_soup.find("p", {"class": "author-name"})
                or talk_soup.find("div", {"class": "body-block"})
            ):
                continue
...
def scrape_talk(args):
    ...
    speaker = (
        clean_author_name(soup.find("p", {"class": "author-name"}).text)
        if soup.find("p", {"class": "author-name"})
        else "No Speaker Found"
    )
    calling = (
        clean_text(soup.find("p", {"class": "author-role"}).text)
        if soup.find("p", {"class": "author-role"})
        else "No Calling Found"
    )
    content_array = soup.find("div", {"class": "body-block"})
    content = (
        "\n\n".join(clean_text(p.text) for p in content_array.find_all("p"))
        if content_array

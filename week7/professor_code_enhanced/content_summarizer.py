...
class ContentSummarizer:
    ...
    def extract_html_content(self, html: str) -> Dict[str, str]:
        ...
        content['main_text'] = ' '.join(main_content[:10])  # Limit to first 10 substantial paragraphs
    ...
    def intelligent_chunk(self, text: str, content_type: str) -> List[str]:
        ...
        if content_type == 'html':
            ...
            section_pattern = r'(<(?:section|article|div)[^>]*>.*?</(?:section|article|div)>)'
    ...
    async def summarize_chunk(self, chunk: str, content_type: str, context: str = "") -> str:
        ...
        if content_type == 'html':
            ...
            prompt = f"Summarize the key data and structure from this JSON content:\n\n{chunk}"
        else:
        ...
        try:
            response = await client.chat.completions.create(
                ...
                messages=[
                    {"role": "system", "content": "You are a content summarizer. Create concise, informative summaries that capture the key points and important details. Be comprehensive but brief."},
    ...
    async def aggregate_summaries(self, summaries: List[str], original_context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if len(combined_text) > self.chunk_size:
            ...
            try:
                response = await client.chat.completions.create(
                    model="gpt-4o-mini",
                    messages=[
                        {"role": "system", "content": "You are synthesizing multiple content summaries into a comprehensive overview. Create a coherent, well-organized summary that captures all key information."},
                        {"role": "user", "content": f"Combine these section summaries into a comprehensive overview:\n\n{combined_text}"}
                    ],
                    temperature=0.3,
                    max_tokens=1200
                )
    ...
    async def process_content(self, text: str, headers: Dict[str, str] = None, context: str = "") -> Dict[str, Any]:
        """
        ...
        for i, chunk in enumerate(chunks):
            chunk_context = f"{context} (Part {i+1} of {len(chunks)})" if context else f"Part {i+1} of {len(chunks)}"
            summary = await self.summarize_chunk(chunk, content_type, chunk_context)
            summaries.append(summary)
        
        # Aggregate summaries
        result = await self.aggregate_summaries(summaries, {'original_size': original_size})
        ...
        result.update({
            ...
            'truncated_preview': text[:1000] + "..." if original_size > 1000 else text
        })
...
async def summarize_content(text: str, headers: Dict[str, str] = None, context: str = "") -> Dict[str, Any]:

...
class PersistentSandboxManager:
    ...
    def __init__(self, max_environments: int = 5, environment_ttl: int = 3600):
        ...
        self.plots_dir = Path("skill_sandbox_plots")
    ...
    async def execute_in_environment(
        ...
        try:
            ...
            if language in {"python", "r"} and result.plots:
                output["plots"] = await self._save_plots(skill_name, result.plots)
    ...
    async def _save_plots(
        self, skill_name: str, plots: List[Any]
    ) -> List[Dict[str, Any]]:
        """Save plots generated by skill execution."""
        saved_plots = []
        skill_plots_dir = self.plots_dir / skill_name
        skill_plots_dir.mkdir(exist_ok=True)

        for i, plot in enumerate(plots):
            timestamp = int(time.time())
            plot_filename = f"plot_{timestamp}_{i+1}.{plot.format.value}"
            plot_path = skill_plots_dir / plot_filename

...
def _get_strict_json_schema_type(annotation) -> dict:
    ...
    if origin is list or annotation is list or origin is List:
        if args:
            # Note: for simplicity, we assume that all list items are the same type
            item_type = args[0]
            item_schema = _get_strict_json_schema_type(item_type)
            return {"type": "array", "items": item_schema}
        else:
            # Default to string if needed
